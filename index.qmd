...
1.  **Breach Methods by Sector/Type of Data (Stacked Bar Chart)**: This chart breaks down breaches by sector or type of data and shows which methods attackers use most. By hovering over each section, you can see details like the method used and how many records were lost. This helps us see which sectors or types of data are most at risk from specific attack methods.

2.  **Breach Methods by Sector/Type of Data (Animated Stacked Bar Chart)**: This one's like a moving version of the first chart. It shows how breach patterns change over time within each sector or type of data. By watching the animation year by year, we can spot trends and see if certain attack methods are becoming more common in specific sectors.

3.  **Breach Severity by Sector/Type of Data (Relative Stacked Bar Chart)**: This chart compares the severity of breaches across different sectors or types of data. It focuses on the proportion of data lost within each category rather than the total number of breaches. This helps us understand which sectors or types of data face the biggest potential losses.

4.  **Records Lost by Sector/Method (Stacked Bar Chart)**: Here, we see the total number of records lost in breaches, grouped by sector and breach method. It complements the first chart by highlighting which sectors suffer the most significant data losses due to specific breach methods.

5.  **Breach Frequency Over Time (Line Chart)**: This animated chart shows how often breaches occur over the years, categorized by the methods used. It helps us track changes in cybercrime activity over time and spot any new trends or emerging threats.

By putting all these insights together, we can:

-   **Identify vulnerable sectors and data types**: Knowing which areas are most at risk allows businesses to focus their efforts where they're needed most.

-   **Understand the severity of breaches**: By comparing data losses across different sectors, we can prioritize security measures and allocate resources accordingly.

-   **Stay ahead of evolving threats**: Tracking temporal trends helps us anticipate changes in attacker tactics and adjust our defenses proactively.

Ultimately, this knowledge helps businesses:

-   **Focus on the most critical vulnerabilities**: By targeting specific sectors, data types, and breach methods, they can strengthen their defenses where it matters most.

-   **Implement tailored security measures**: Armed with insights into the most prevalent threats, businesses can deploy security controls that address their unique risks.

-   **Regularly assess and improve their defenses**: By staying vigilant and adapting to changing threats, businesses can minimize the impact of cyberattacks and protect their data and stakeholders effectively.
...
# Aggregate data by sector and method
agg_data_sum <- agg_data %>%
  group_by(sector, method) %>%
  summarise(records_lost = sum(records_lost)) %>%
  ungroup() %>%
  mutate(proportion_lost = records_lost / sum(records_lost))  # Calculate proportion of records lost

# Create a relative stacked bar chart
bar_chart <- plot_ly(data = agg_data_sum,
                     x = ~sector,
                     y = ~proportion_lost,
                     type = 'bar',
                     color = ~method,
                      colors = 'Dark2',
                     hoverinfo = 'text',
                     text = ~paste('Method:', method,
                                   '<br>Proportion of Records Lost:', scales::percent(proportion_lost))) %>%
  layout(
    barmode = 'stack',
    title = 'Breach Severity by Sector/Type of Data (Relative Stacked Bar Chart)',
    xaxis = list(title = 'Sector/Type'),
    yaxis = list(title = 'Proportion of Records Lost'),
    margin = list(b = 150),
    showlegend = TRUE
  ) %>%
  config(displayModeBar = TRUE)
...
# Assuming agg_data contains data for multiple years
# and has columns: 'sector', 'method', 'year', 'records_lost'

# Aggregate data by year
agg_data_year <- agg_data %>%
  group_by(year) %>%
  summarise(total_records_lost = sum(records_lost))

# Create a scatter plot
scatter_plot <- plot_ly(agg_data_year, x = ~year, y = ~total_records_lost, type = "scatter", mode = "markers",
                        marker = list(color = "red", size = 10),
                        text = ~paste("Year:", year, "<br>Records Lost:", total_records_lost),
                        hoverinfo = "text") %>%
  layout(title = "Total Records Lost Over Time",
         xaxis = list(title = "Year"),
         yaxis = list(title = "Total Records Lost"),
         showlegend = FALSE) %>%
  config(displayModeBar = TRUE)
