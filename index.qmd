...
## top 10 plot sector (raching bar chart)
...
names(agg_data)[names(agg_data) == 'year   '] <- 'year'
...
# Aggregate data
agg_data_sum <- df_decade %>%
  group_by(year, method) %>%
  summarise(breach_count = n(), .groups = 'drop')

# Assign colors to different breach types
color_palette <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf")
breach_types <- unique(agg_data_sum$method)
breach_colors <- setNames(color_palette[1:length(breach_types)], breach_types)
