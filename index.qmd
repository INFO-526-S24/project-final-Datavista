...
agg_data_sector <- agg_data %>%
  group_by(sector) %>%
  summarise(records_lost = sum(records_lost)) %>%
  ungroup() %>%
  arrange(desc(records_lost))

# Now we are  aggregate small sectors into 'Other'
cutoff <- 0.01  # Threshold for smallest percentage to show separately
total_records <- sum(agg_data_sector$records_lost)
agg_data_sector <- agg_data_sector %>%
  mutate(percentage = records_lost / total_records) %>%
  mutate(sector = ifelse(percentage < cutoff, "Other", as.character(sector)))

# Combining all 'Other' categories into one
agg_data_sector <- agg_data_sector %>%
  group_by(sector) %>%
  summarise(records_lost = sum(records_lost)) %>%
  ungroup()

plot <- plot_ly(agg_data_sector, labels = ~sector, values = ~records_lost, type = 'pie',
                textinfo = 'label+percent', insidetextorientation = 'radial') %>%
  layout(title = 'Total Records Lost by Sector', showlegend = TRUE)
...
# Assuming agg_data is your data frame with the columns: sector, method, records_lost, year
...
fig <- plot_ly(data = agg_data_sum, x = ~sector, y = ~records_lost, type = 'bar', name = ~method,
  ...
  config(displayModeBar = TRUE) # Enable the display mode bar for zoom and other tools
